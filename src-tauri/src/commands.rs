...
pub async fn download_all_favorites(app: AppHandle) -> CommandResult<()> {
    ...
    let concurrency = config.read().download_all_favorites_concurrency;
    ...
    let total_comics = favorite_comics.len() as i64;
    let _ = DownloadAllFavoritesEvent::GetFavoritesEnd.emit(&app);

    // ===== 并发获取漫画详情 =====
    let comic_sem = Arc::new(Semaphore::new(concurrency));
    let app_clone = app.clone();

    // 并发处理每个漫画
    let mut comic_join_set = JoinSet::new();
    for (idx, favorite_comic) in favorite_comics.into_iter().enumerate() {
        let comic_sem = comic_sem.clone();
        let app_clone = app_clone.clone();
        let interval_sec = interval_sec;

        comic_join_set.spawn(async move {
            let _permit = comic_sem.acquire().await.map_err(|e| anyhow::anyhow!("{}", e))?;

            let comic_title = favorite_comic.name.clone();
            let comic_id: i64 = favorite_comic.id.parse().map_err(|e| anyhow::anyhow!("{}", e))?;

            // 获取漫画详情 (带重试)
            let comic = match get_comic_with_retry(&app_clone, comic_id, 3).await {
                Ok(c) => c,
                Err(err) => {
                    let err_title = format!("获取漫画`{}`详情失败，已跳过", comic_title);
                    tracing::error!(err_title, message = err.to_string());
            ...
            let _ = DownloadAllFavoritesEvent::GetComicsProgress { current, total: total_comics }.emit(&app_clone);

            // 过滤未下载的章节
            let chapter_infos: Vec<ChapterInfo> = comic
                .chapter_infos
                .into_iter()
                .filter(|ci| ci.is_downloaded != Some(true))
                .collect();
            ...
            let _ = DownloadAllFavoritesEvent::EndCreateDownloadTasks { comic_id: comic.id }.emit(&app_clone);
    ...
    // 等待所有漫画处理完成
    while let Some(result) = comic_join_set.join_next().await {
        if let Err(e) = result {
            tracing::error!("并发处理收藏夹漫画时出错: {}", e);
        }
    }
...
async fn get_comic_with_retry(
    ...
    for attempt in 0..max_retries {
        match utils::get_comic(app.clone(), comic_id).await {
            Ok(comic) => return Ok(comic),
            Err(e) => {
                last_error = Some(e);
                if attempt < max_retries - 1 {
                    let wait_secs = 2_i64.pow(attempt);
                    tracing::warn!("获取漫画详情失败，{}秒后重试 (尝试 {}/{})", wait_secs, attempt + 1, max_retries);
    ...
    Err(anyhow::anyhow!("获取漫画详情失败，已重试{}次: {:?}", max_retries, last_error))
}
